...
class MainActivity : AppCompatActivity() {
    ...
    fun addStudent(v: View) {
        ...
        mDialogView.btnAdd.setOnClickListener {
            ...
            var result =
                dbHandler?.insertStudent(Student(std_id = id, std_name = name, std_age = age))
            if (result!! > -1) {
                Toast.makeText(
                    applicationContext,
    ...
    fun editDeleteDialog(position: Int) {
        ...
        mDialogView.edit_id.setText(std.std_id)
        mDialogView.edit_id.isEnabled = false
        mDialogView.edit_name.setText(std.std_name)
        mDialogView.edit_age.setText(std.std_age.toString())
        ...
        mDialogView.btnUpdate.setOnClickListener {
            ...
            var name = mDialogView.edt_name.text.toString()
            var age = mDialogView.edt_age.text.toString().toInt()
            var result = dbHandler?.UpdateStudent(Student(std_id = id, std_name = name, std_age = age))
            if (result!! > -1) {
                Toast.makeText(
